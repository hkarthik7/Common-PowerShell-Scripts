...
<#
    ...
    .NOTES
        ...
        harish.b.karthic		    v1.2			19/09/2019		Bug fixes and merged Windows and DB checks
#>
...
begin {
    ...
    $FailSafeReport = "FailSafes.htm"
    $FailSafePath = "$ExportPath\$FailSafeReport"
...
"@
    ...
    $Output = "
        <HTML>
    	<TITLE> WINDOWS AND DB COMPARISON RESULTS </TITLE>
    	<BODY background-color:peachpuff>
       	<font color =""#B03A2E"" face=""Microsoft Tai le"">
       	<H1> WINDOWS AND DB COMPARISON RESULTS </H1>
       	<H3> Base VM Image comparison post build <br></H3>
        <H4> <a href=$("FailSafes.htm")>FAILSAFES</a> <br></H4>
    	</font>
    ...
    $head = @"
        ...
        TD{border-width: 1px; padding: 3px; border-style: solid; border-color: black; background-color: lightgrey}
        </STYLE>
"@
...
process {
    ...
    Foreach($OSData in $WindowsOSData) {
        ...
        #region validation extraction for failsafes

        $results = [PSCustomObject]@{
            Name = $Name
            TimeFormat = $TimeFormat
            CrowdStrikeDeviceControl = $CrowdStrikeDeviceControl
            CrowdStrikePNPDrivers = $CrowdStrikePNPDrivers
            CrowdStrikeSensor = $CrowdStrikeSensor
            FileSystem = $FileSystem
            Mellanox = $Mellanox
            ODBC = $ODBC
            OMSAgentSoftware = $OMSAgentSoftware
            RedCloakRegistry = $RedCloakRegistry
            RedCloakSoftware = $RedCloakSoftware
        ...
        #$results = $results | ConvertTo-Html -Body "<H3>FAILSAFES</H3>" -Head $head | Out-File $FailSafePath -Append
        $FailSafes += $results

        #endregion validation extraction for failsafes
        ...
        $CSV += $Hash
            ...
            If($Hash.'Windows Defender' -eq $true) {
                ...
                $Output += "
                    <TD align='center' bgcolor=#17A589>$($WindowsDefender)</TD>
                "
    ...
    $FailSafes | ConvertTo-Html -Body "<H3>FAILSAFES</H3>" -Head $head | Out-File $FailSafePath
...
Compare-WindowsChecks -InputfilePath "C:\Users\h.i.balasubramanian\OneDrive - Accenture\Mondelez\WindowsChecks\WindowsOSChecks-18-09-2019.csv" `
                      -ExportPath "C:\Users\h.i.balasubramanian\OneDrive - Accenture\Mondelez\Comparison" `
                      -Verbose