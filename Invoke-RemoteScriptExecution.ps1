                ...
                Sequence { 
                   ...
                   $StartTime = Get-Date -UFormat %Y/%m/%d_%H:%M:%S
                    ...
                    if ($Computer.Name -ne $null) {
                        ...
                        $Destination = $ArgumentList.Replace(":","$")
                        InlineScript {
                            ...
                            If ($JobCompleted -ne $null) {
                                ...
                                Copy-Item "\\$($using:ComputerName)\$($using:Destination)\*-*.csv" -Destination $using:ExportPath
...
Function Merge-CSVs {
    ...
    process {
        try {
            ...
            $CSV | Export-CSV -Path "$OutputPath\$OutputName.csv" -NoTypeInformation -UseCulture
...
$OutputPath = "C:\Temp\Mondelez\WindowsChecks"

$ScriptPathWithArguments = @{
        Script1 = @{ScriptPath = "C:\Temp\Mondelez\Scripts\Get-ServerData.ps1"
                ArgumentList = "C:\Temp"
               }
...
#Merge CSVs
$FilenamePattern = @("AdministratorGroupMembers","ClusterGroup","ClusterNode","ComputerSystem","DefaultIPGateway","Disks",
"DNS","DnsClientGlobalSetting","DomainSubnets","Drivers","InstalledSoftwares","KeepAliveTimeInterval","LocalUser",
"NetAdapterAdvancedProperty","NetIPInterface","OperatingSystem","PageFile","ResolveDNSName","Services","TimeZone",
"WindowsActivation","WindowsFeature","WindowsFirewall","WindowsPatches")

Foreach($Pattern in $FilenamePattern) {
    Merge-CSVs -InputPath $Path -Pattern "*-$Pattern.csv" -OutputPath $OutputPath -OutputName "AllServerData-$($Pattern)" -Verbose
}
